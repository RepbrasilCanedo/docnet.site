 ...
 public function index(string|int|null $page = null): void
    {
        ...
        $listTicketSla = new \App\cpms\Models\CpmsRelatListSla();
        
        if ((!empty($this->dataForm['SendSearchSla']))) {
            $this->page = 1;
            $listTicketSla->listSearchSla($this->page, $this->dataForm['search_ticket'], $this->dataForm['search_empresa'], $this->dataForm['search_status'], $this->dataForm['search_status_anterior'], $this->dataForm['search_tipo'], $this->dataForm['search_date_start'], $this->dataForm['search_date_end'], $this->dataForm['search_suporte']);
            $this->data['form'] = $this->dataForm;

        // Busca pelo numero do Ticket    
        } elseif ((!empty($this->searchTicket))) {
            $listTicketSla->listSearchSla($this->page, $this->searchTicket, $this->searchEmpresa, $this->searchTipo, $this->searchStatus, $this->searchStatusAnterior, $this->searchDateStart, $this->searchDateEnd, $this->searchSuporte);
        ...
        // Busca pelo tipo do Sla do Ticket
            ...
            $listTicketSla->listSearchSla($this->page, $this->searchTicket, $this->searchEmpresa, $this->searchStatus, $this->searchStatusAnterior, $this->searchTipo, $this->searchDateStart, $this->searchDateEnd, $this->searchSuporte);
        ...
        // Busca por ambos status do Ticket
        } elseif ((empty($this->searchEmpresa)) or (!empty($this->searchStatus))or (!empty($this->searchStatusAnterior))  or (empty($this->searchTipo)) or (empty($this->searchDateStart)) or (empty($this->searchDateEnd)) or (empty($this->searchSuporte))) {
            $listTicketSla->listSearchSla($this->page, $this->searchTicket, $this->searchEmpresa, $this->searchStatus, $this->searchStatusAnterior, $this->searchTipo, $this->searchDateStart, $this->searchDateEnd, $this->searchSuporte);
            $this->data['form']['search_status_anterior'] = $this->searchStatusAnterior;
            $this->data['form']['search_status'] = $this->searchStatus;
        ...
        $button = ['relat_list_sla' => ['menu_controller' => 'relat-list-sla', 'menu_metodo' => 'index']];

        $listBotton = new \App\adms\Models\helper\AdmsButton();
        $this->data['button'] = $listBotton->buttonPermission($button);
        
        $listSelect = new \App\cpms\Models\CpmsRelatListSla();
        $this->data['select'] = $listSelect->listSelect();

        $listMenu = new \App\adms\Models\helper\AdmsMenu();
        $this->data['menu'] = $listMenu->itemMenu(); 

        $this->data['sidebarActive'] = "relat-list-sla";         
        $loadView = new \Core\ConfigView("cpms/Views/sla/relatListSla", $this->data);
        $loadView->loadView();
    }
