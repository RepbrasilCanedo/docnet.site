...
/**
    ...
    public function listSearchCham(int $page, string|null $search_ticket, string|null $search_empresa, string|null $search_status, string|null $search_status_anterior, string|null $search_Tipo, string|null $search_Date_Start, string|null $search_Date_End, string|null $search_suporte): void
    {
        ...
        if (!empty($this->searchTicket)) {
            ...
            if ((!empty($this->searchEmpresa)) and (empty($this->searchStatus)) and (empty($this->searchTipo)) and (empty($this->searchDateStart)) and (empty($this->searchDateEnd)) and (empty($this->searchSuporte))) {
                ...
                $this->searchSupPerStaAnt(); //Pesquisa pelo Suporte, periodo e  status anterior do atendimento
            } else if ((empty($this->searchEmpresa)) and (!empty($this->searchStatus)) and (!empty($this->searchStatusAnterior)) and (empty($this->searchTipo)) and (!empty($this->searchDateStart)) and (!empty($this->searchDateEnd)) and (!empty($this->searchSuporte))) {
    ...
    /**
     * Metodo pesquisar pelo suporte e periodo e status anterior do Ticket
     * @return void
     */
    public function searchSupPerStaAnt(): void
    {
        if ($_SESSION['adms_access_level_id'] > 2) {
            ...
            if ($_SESSION['adms_access_level_id'] == 4)  {
                ...
                $pagination = new \App\adms\Models\helper\AdmsPagination(URLADM . 'list-ticket-sla/index');
                $pagination->condition($this->page, $this->limitResult);
                $pagination->pagination("SELECT COUNT(id_ticket) AS num_result FROM adms_sla_hist WHERE (empresa_id= :empresa_id) AND (status_id_ant= :status_id_ant) AND (suporte_id = :suporte_id) AND (dt_status BETWEEN :search_date_start AND :search_date_end)", 
                "empresa_id={$_SESSION['emp_user']}&status_id_ant={$this->searchStatusAnteriorValue}&suporte_id={$this->searchSuporteValue}&search_date_start={$this->searchDateStart}&search_date_end={$this->searchDateEnd}");
                $this->resultPg = $pagination->getResult();
                ...
                $listTicketSla->fullRead("SELECT sla_hist.id AS id_sla_hist, sla_hist.empresa_id, clie.nome_fantasia AS nome_fantasia_clie, sla_hist.id_ticket AS id_ticket_sla_hist, sla_hist.dt_abert_ticket as dt_abert_ticket, sla_hist.type_cham, sla_hist.dt_status as dt_status, 
                            ...
                            INNER JOIN adms_cham_status AS sta_atu ON sta_atu.id = sla_hist.status_id
                            INNER JOIN adms_users AS user ON user.id = sla_hist.suporte_id 
                            WHERE (sla_hist.empresa_id= :empresa_id) AND (status_id_ant= :status_id_ant) AND (sla_hist.suporte_id = :suporte_id) AND (dt_status BETWEEN :search_date_start AND :search_date_end) ORDER BY sla_hist.dt_status DESC LIMIT :limit OFFSET :offset",
                            "empresa_id={$_SESSION['emp_user']}&status_id_ant={$this->searchStatusAnteriorValue}&suporte_id={$this->searchSuporteValue}&search_date_start={$this->searchDateStart}&search_date_end={$this->searchDateEnd}&limit={$this->limitResult}&offset={$pagination->getOffset()}");
    ...
    /**
     * Metodo pesquisar pelo suporte e periodo e status anterior e status atual do Ticket
     * @return void
     */
    public function searchSupPerStaAtual(): void
    {
        if ($_SESSION['adms_access_level_id'] > 2) {
            ...
            if ($_SESSION['adms_access_level_id'] == 4)  {
                ...
                $pagination = new \App\adms\Models\helper\AdmsPagination(URLADM . 'list-ticket-sla/index');
                $pagination->condition($this->page, $this->limitResult);
                $pagination->pagination("SELECT COUNT(id_ticket) AS num_result FROM adms_sla_hist WHERE (empresa_id= :empresa_id) AND (status_id_ant= :status_id_ant) AND (status_id= :status_id) AND (suporte_id = :suporte_id) AND (dt_status BETWEEN :search_date_start AND :search_date_end)", 
                "empresa_id={$_SESSION['emp_user']}&status_id_ant={$this->searchStatusAnteriorValue}&status_id={$this->searchStatusValue}&suporte_id={$this->searchSuporteValue}&search_date_start={$this->searchDateStart}&search_date_end={$this->searchDateEnd}");
                $this->resultPg = $pagination->getResult();
                ...
                $listTicketSla->fullRead("SELECT sla_hist.id AS id_sla_hist, sla_hist.empresa_id, clie.nome_fantasia AS nome_fantasia_clie, sla_hist.id_ticket AS id_ticket_sla_hist, sla_hist.dt_abert_ticket as dt_abert_ticket, sla_hist.type_cham, sla_hist.dt_status as dt_status, 
                            ...
                            INNER JOIN adms_cham_status AS sta_atu ON sta_atu.id = sla_hist.status_id
                            INNER JOIN adms_users AS user ON user.id = sla_hist.suporte_id 
                            WHERE (sla_hist.empresa_id= :empresa_id) AND (status_id_ant= :status_id_ant) AND (status_id= :status_id) AND (sla_hist.suporte_id = :suporte_id) AND (dt_status BETWEEN :search_date_start AND :search_date_end) ORDER BY sla_hist.dt_status DESC LIMIT :limit OFFSET :offset",
                            "empresa_id={$_SESSION['emp_user']}&status_id_ant={$this->searchStatusAnteriorValue}&status_id={$this->searchStatusValue}&suporte_id={$this->searchSuporteValue}&search_date_start={$this->searchDateStart}&search_date_end={$this->searchDateEnd}&limit={$this->limitResult}&offset={$pagination->getOffset()}");
    ...
    /**
     * Metodo pesquisar pelo suporte e periodo e status anterior e status atual e cliente do Ticket
     * @return void
     */
    public function searchSupPerStaAtuCli(): void
    {
        if ($_SESSION['adms_access_level_id'] > 2) {
            ...
            if ($_SESSION['adms_access_level_id'] == 4)  {
                ...
                $pagination = new \App\adms\Models\helper\AdmsPagination(URLADM . 'list-ticket-sla/index');
                $pagination->condition($this->page, $this->limitResult);
                $pagination->pagination("SELECT COUNT(id_ticket) AS num_result FROM adms_sla_hist WHERE (empresa_id= :empresa_id) AND (cliente_id = :cliente_id) AND (status_id_ant= :status_id_ant) AND (status_id= :status_id) AND (suporte_id = :suporte_id) AND (dt_status BETWEEN :search_date_start AND :search_date_end)", 
                "empresa_id={$_SESSION['emp_user']}&cliente_id={$this->searchEmpresaValue}&status_id_ant={$this->searchStatusAnteriorValue}&status_id={$this->searchStatusValue}&suporte_id={$this->searchSuporteValue}&search_date_start={$this->searchDateStart}&search_date_end={$this->searchDateEnd}");
                $this->resultPg = $pagination->getResult();
