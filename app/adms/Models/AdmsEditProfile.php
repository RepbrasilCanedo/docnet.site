...
class AdmsEditProfile
{
    ...
    public function viewProfile(): void
    {
        if ($_SESSION['adms_access_level_id'] == 14) {
            ...
            $viewUser->fullRead("SELECT name, nickname, email, tel_1, image, user
                                FROM adms_users_final
                                WHERE id=:id
            ...
            $viewUser = new \App\adms\Models\helper\AdmsRead();
            $viewUser->fullRead(
                "SELECT id, name, nickname, email, tel_1, tel_2, user 
                            FROM adms_users
                            WHERE id=:id
                            LIMIT :limit",
     ...
     * Salva os campos name, nickname e email na sessão
     * @return void
     */
